...
const Orders = () => {
    ...
    useFocusEffect(
        useCallback(() => {
            const loadUserDataAndFetchOrders = async () => {
                try {
                    ...
                    const sortedOrders = (response.data || [])
                        .filter((order: any) => {
                            const firstItem = order?.orderItemsV1DTOS?.[0];
                            const status = firstItem?.orderStatusName || '';
                            // Only include orders that are NOT PENDING or PAYMENT_FAILURE
                            return status !== 'PENDING' && status !== 'PAYMENT_FAILURE';
                        })
                        .sort(
                            (a: any, b: any) =>
    ...
    const renderItem = ({ item }: { item: Order }) => {
        ...
        return (
            <TouchableOpacity
                activeOpacity={0.8}
                onPress={() =>
                    navigation.navigate('SeparateProductPage', { productId: firstItem.productId })
                }
            >
                    ...
                    <View style={styles.content}>
                        ...
                        <View style={styles.info}>
                            ...
                            <Text>
                                ...
                                <Text style={styles.price}>₹{item?.total ?? 0}</Text>
                            </Text>
...
const styles = StyleSheet.create({
    ...
    image: { width: 65, height: 85, borderRadius: 8, marginRight: 12 },
    info: { flex: 1 },
    product: { fontSize: 15, fontWeight: 'bold' },
    price: { fontSize: 15, color: '#007bff', marginVertical: 4 },
